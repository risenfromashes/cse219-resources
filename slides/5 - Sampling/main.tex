...
\begin{frame}{DTFT Revisited from the Sampled Signal}
  Earlier in the \textbf{Recap on DFT} we derived the CTFT of the sampled impulse train:
  \begin{equation*}
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      \begin{align*}
        X_p(j(\omega+\omega_s))
        &= \sum_{n=-\infty}^{\infty} x[n]e^{-j(\omega+\omega_s)nT} \\
        &= \sum_{n=-\infty}^{\infty} x[n]e^{-j\omega nT}e^{-j2\pi n} \\
        &= X_p(j\omega)
      \end{align*}
      \begin{itemize}
          \setlength{\itemsep}{0.15em}
        \item This is exactly the sampled spectrum from the Nyquist proof.
        \item It repeats every $\omega_s=\frac{2\pi}{T}$.
        \item Under $\omega T \mapsto \omega$, the same expression becomes the DTFT:
          \begin{equation*}
    ...
    \begin{column}{0.49\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            ...
            axis lines=middle, axis line style={-{Latex}},
            xmin=-10.5, xmax=10.5, ymin=-0.25, ymax=2.25,
            xtick={-8,-4,0,4,8},
            ...
            declare function={
              gauss(\x)=1.35*exp(-(\x*\x)/8);
              xp(\x)=gauss(\x)+gauss(\x-8)+gauss(\x+8);
            }
          ]
          \fill[PrimaryColor!30,opacity=0.5] (axis cs:-4,0) rectangle (axis cs:4,1.45);
          \fill[AccentColor!30,opacity=0.5] (axis cs:-12,0) rectangle (axis cs:-4,1.45);
          \fill[AccentColor!30,opacity=0.5] (axis cs:4,0) rectangle (axis cs:12,1.45);
          \addplot[samples=180, domain=-10:10, very thick, PrimaryColor] {xp(x)};
          \draw[<->, thick, gray,font=\scriptsize] (axis cs:-4,1.72) -- node[above] {one period $=\omega_s$} (axis cs:4,1.72);
          \node[font=\scriptsize, AccentColor] at (axis cs:7.2,0.2) {shifted copy};
          \node[font=\scriptsize, AccentColor] at (axis cs:-7.2,0.2) {shifted copy};
        \end{axis}
      \end{tikzpicture}
    \end{column}
...
\begin{frame}{Baseband Copy, Aliasing, and the Ideal LPF}
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      \small
      \begin{equation*}
      ...
      \begin{itemize}
        ...
        \item The central band $[-\omega_s/2,\omega_s/2]$ contains the original copy centered at $0$.
        \item Every other interval of width $\omega_s$ is a shifted copy of that same spectrum.
        \item For finite-duration captures the tails are infinite, so some aliasing leakage remains, but the baseband picture is still the same one from the Nyquist proof.
        \item The ideal reconstruction filter keeps exactly that baseband copy:
          \begin{equation*}
            H(j\omega)=
            \begin{cases}
              T, & |\omega|<\omega_s/2 \\
    ...
    \begin{column}{0.51\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            width=\textwidth, height=5.55cm,
            axis lines=middle, axis line style={-{Latex}},
            xmin=-10.5, xmax=10.5, ymin=-0.2, ymax=2.35,
            xtick={-8,-4,0,4,8},
            xticklabels={$-\omega_s$, $-\omega_s/2$, $0$, $\omega_s/2$, $\omega_s$},
            ytick=\empty, xlabel={$\omega$}, ylabel={$|X_p(j\omega)|$}, clip=false,
            xlabel style={anchor=west, xshift=10pt},
            ylabel style={anchor=south},
            declare function={
              widegauss(\x)=1.28*exp(-(\x*\x)/10);
              xp(\x)=widegauss(\x)+widegauss(\x-8)+widegauss(\x+8);
            }
          ]
          \fill[PrimaryColor!10] (axis cs:-4,0) rectangle (axis cs:4,1.42);
          \addplot[samples=180, domain=-10:10, very thick, PrimaryColor] {xp(x)};
          \addplot[SecondaryColor, thick, dashed, samples=120, domain=-4:4] {widegauss(x)};
          \draw[very thick, SecondaryColor] (axis cs:-4,1.56) -- (axis cs:-4,1.94) -- (axis cs:4,1.94) -- (axis cs:4,1.56);
          \node[SecondaryColor, font=\bfseries\scriptsize] at (axis cs:0,2.08) {ideal LPF keeps this band};
          \node[font=\scriptsize] at (axis cs:0,1.56) {original copy};
          \node[font=\scriptsize, AccentColor] at (axis cs:7.2,0.2) {shifted copy};
          \node[font=\scriptsize, AccentColor] at (axis cs:-7.2,0.2) {shifted copy};
        \end{axis}
      \end{tikzpicture}
...
\begin{frame}{Inverse DTFT from the Ideal LPF}
  \begin{columns}[T]
    \begin{column}{0.38\textwidth}
      \begin{itemize}
          \setlength{\itemsep}{0.3em}
        \item Ideal reconstruction uses one $\omega_s$-wide period of the sampled spectrum:
          \begin{equation*}
            x(t)=\frac{1}{2\pi}\int_{-\omega_s/2}^{\omega_s/2} X_p(j\omega)e^{j\omega t}\,Td\omega
          \end{equation*}
        \item After the same mapping $\omega T \mapsto \omega$  used to derive the DTFT,
          \begin{align*}
            X_p(j\omega) & \longrightarrow X(e^{j\omega}) \\
            T d\omega &\longrightarrow d\omega
          \end{align*}

        \item Also,
          \begin{center}
            \begin{tikzpicture}[ scale=0.45]
              \node at (0,0) {$[-\omega_s/2,\omega_s/2]$};
              \draw[->, thick] (2.3,0) -- node[above] {$\omega T \mapsto \omega$} (5.6,0);
              \node at (6.8,0) {$[-\pi,\pi]$};
            \end{tikzpicture}
          \end{center}
      \end{itemize}
    ...
    \begin{column}{0.58\textwidth}
      \small
      Applying the mapping at the sample locations,
      \begin{align*}
        x[n]=x(nT)
        &= \frac{1}{2\pi}\int_{-\pi}^{\pi} X(e^{j\omega})e^{j\frac{\omega}{T} Tn}\,d\omega \\
        &= \frac{1}{2\pi}\int_{-\pi}^{\pi} X(e^{j\omega})e^{j\omega n}\,d\omega
      \end{align*}
      \vspace{0.04cm}
      \begin{conceptbox}{Inverse DTFT}
        \vspace{-0.2cm}
        \begin{equation*}
          x[n]=\frac{1}{2\pi}\int_{2\pi} X(e^{j\omega})e^{j\omega n}\,d\omega
        \end{equation*}
      \end{conceptbox}
      As the area is the same regardless of which period we choose.
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{DFT as Uniform Samples of One $\omega_s$-Wide Period}
  \begin{equation*}
  ...
  \begin{columns}[T]
    \begin{column}{0.43\textwidth}
      ...
      \begin{itemize}
        ...
        \item Undoing the relabeling, a normalized DTFT point at $\omega$ sits at the physical frequency $\omega/T=\omega\omega_s/(2\pi)$.
        \item So an $N$-point DFT takes $N$ uniformly spaced samples across one physical period of width $\omega_s$.
        \item For even $N$, bins $k=0,\dots,N/2$ run from dc to the highest positive frequency.
        \item Bins $k=N/2+1,\dots,N-1$ then continue as wrapped negative frequencies: large negative first, then back toward dc.
      \end{itemize}
    \end{column}
    ...
    \begin{column}{0.53\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            width=\textwidth, height=5.15cm,
            axis lines=middle, axis line style={-{Latex}},
            xmin=0, xmax=8.6, ymin=-0.36, ymax=1.67,
            xtick={0,4,8},
            xticklabels={$0$, $\omega_s/2$, $\omega_s$},
            ytick=\empty, xlabel={$\omega$}, ylabel={$|X_p(j\omega)|$}, clip=false,
            xlabel style={anchor=west, xshift=10pt},
            ylabel style={anchor=south},
            declare function={
              gauss(\x)=1.25*exp(-(\x*\x)/6);
              xp(\x)=(\x <= 4) ? gauss(\x) : gauss(\x-8);
            }
          ]
          \addplot[samples=150, domain=0:8, thick, PrimaryColor!40] {xp(x)};
          \addplot[ycomb, mark=*, mark size=1.25pt, thick, AccentColor, samples=16, domain=0:7.5] {xp(x)};
          \draw[dashed, DarkGray, thin] (axis cs:4,-0.28) -- (axis cs:4,1.42);
          \draw[<->] (axis cs:0,-0.6) -- node[below] {$\omega_s/N$} (axis cs:0.5,-0.6);
          \draw[<->, thick, PrimaryColor] (axis cs:0,-0.44) -- node[below, font=\scriptsize] {positive} (axis cs:4,-0.44);
          \draw[<->, thick, AccentColor] (axis cs:4,-0.44) -- node[below, font=\scriptsize, align=center] {wrapped\\negative} (axis cs:8,-0.44);
          \node[AccentColor, font=\scriptsize] at (axis cs:2.15,1.38) {DFT evaluates $N$ bins};
        \end{axis}
      \end{tikzpicture}
    \end{column}
  \end{columns}
...
\begin{frame}{Periodic Signals Give Harmonic Line Spectra}
  \begin{columns}[T]
    \begin{column}{0.43\textwidth}
      \begin{itemize}
          \setlength{\itemsep}{0.34em}
        \item If the underlying signal has period $T_0$, then $\omega_0=2\pi/T_0$ and
          \begin{equation*}
            x(t)=\sum_{k=-\infty}^{\infty} a_k e^{jk\omega_0 t}.
          \end{equation*}
        \item Its CTFT is therefore a line spectrum:
          \begin{equation*}
            X(j\omega)=2\pi\sum_{k=-\infty}^{\infty} a_k \delta(\omega-k\omega_0).
          \end{equation*}
        \item If we sample exactly $N$ times in one period, then
          \begin{equation*}
            T_0=NT, \qquad \omega_s=\frac{2\pi}{T}=N\omega_0.
          \end{equation*}
      \end{itemize}
    \end{column}
    ...
    \begin{column}{0.53\textwidth}
      \centering
      \textbf{One Period in Time} \\ \smallskip
      \begin{tikzpicture}
        \begin{axis}[
            width=\textwidth, height=3.5cm,
            axis lines=middle, axis line style={-{Latex}},
            xmin=-15, xmax=15, ymin=-1.2, ymax=1.5,
            xtick={-10,0,10}, xticklabels={$-T_0$, $0$, $T_0$},
            ytick=\empty, xlabel={$t$}, clip=false,
            xlabel style={anchor=west, xshift=10pt},
            declare function={
      ...
      \vspace{0.08cm}
      \textbf{Harmonic Line Spectrum} \\ \smallskip
      \begin{tikzpicture}
        \begin{axis}[
            width=\textwidth, height=3.55cm,
            axis lines=middle, axis line style={-{Latex}},
            xmin=-8, xmax=8, ymin=-0.2, ymax=2.0,
            xtick={0,2}, xticklabels={$0$, $\omega_0$},
            ytick=\empty, xlabel={$\omega$}, ylabel={$X(j\omega)$}, clip=false,
            xlabel style={anchor=west, xshift=10pt},
            ylabel style={anchor=south},
            declare function={gauss(\x)=1.3*exp(-(\x*\x)/6);}
          ]
          \addplot[ycomb, mark=triangle*, thick, PrimaryColor, samples=5, domain=0:4, mark size=2pt] {gauss(x)};
          \addplot[ycomb, mark=triangle*, thick, AccentColor, samples=4, domain=-4:-1, mark size=2pt] {gauss(x)};
        \end{axis}
      \end{tikzpicture}
    \end{column}
  \end{columns}
...
\begin{frame}{Sampling Copies the Periodic Line Spectrum}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \small
      From the Nyquist proof, the sampled spectrum is a sum of shifted copies of the original spectrum:
      \begin{align*}
        X_p(j\omega)
        &= \frac{1}{T}\sum_{r=-\infty}^{\infty} X(j(\omega-r\omega_s)) \\
        &= \frac{1}{T}\sum_{r=-\infty}^{\infty}
        2\pi\sum_{k=-\infty}^{\infty} a_k
        \delta((\omega-r\omega_s)-k\omega_0) \\
        &= \frac{2\pi}{T}\sum_{r=-\infty}^{\infty}\sum_{k=-\infty}^{\infty}
        a_k\delta(\omega-k\omega_0-r\omega_s)
      \end{align*}
      \begin{itemize}
          \setlength{\itemsep}{0.25em}
        \item Each harmonic line is copied to every passband.
        \item Depending on the bandwidth, the copied line spectra may overlap and alias into each other.
      \end{itemize}
    \end{column}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            width=\textwidth, height=5.7cm,
            axis lines=middle, axis line style={-{Latex}},
            xmin=-8, xmax=16, ymin=-0.2, ymax=2.0,
            xtick={0,8}, xticklabels={$0$, $\omega_s$},
            ytick=\empty, xlabel={$\omega$}, ylabel={$X_p(j\omega)$}, clip=false,
            xlabel style={anchor=west, xshift=10pt},
            ylabel style={anchor=south},
            declare function={amp(\x)=1.3*exp(-(\x*\x)/6);}
          ]
          \addplot[ycomb, mark=triangle*, thick, PrimaryColor, samples=6, domain=0:5, mark size=2pt] {amp(x)};
          \addplot[ycomb, mark=triangle*, thick, AccentColor, samples=5, domain=-5:-1, mark size=2pt] {amp(x)};
          \addplot[ycomb, mark=triangle*, thick, PrimaryColor!35, samples=6, domain=8:13, mark size=2pt] {amp(x-8)};
          \addplot[ycomb, mark=triangle*, thick, AccentColor!35, samples=5, domain=3:7, mark size=2pt] {amp(x-8)};
          \fill[SecondaryColor!12] (axis cs:3,0) rectangle (axis cs:5,1.5);
          \node[SecondaryColor!80!black, font=\scriptsize, align=center] at (axis cs:4,1.65) {possible\\overlap};
        \end{axis}
      \end{tikzpicture}
    \end{column}
  \end{columns}
...
\begin{frame}{Nyquist Condition for the Periodic Case}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \small
      If the original periodic signal is bandlimited to the Nyquist band,
      \begin{equation*}
        |k\omega_0|<\omega_s/2,
      \end{equation*}
      then $a_k=0$ whenever $|k|\ge N/2$. The copied line spectrum becomes
      \begin{equation*}
        X_p(j\omega)=\frac{2\pi}{T}\sum_{r=-\infty}^{\infty}\sum_{|k|<N/2}
        a_k\delta(\omega-k\omega_0-r\omega_s).
      \end{equation*}
      \begin{itemize}
          \setlength{\itemsep}{0.25em}
        \item Now one passband contains one clean, non-overlapping copy of the original harmonic line spectrum.
        \item This is the periodic-signal version of the Nyquist condition.
      \end{itemize}
    \end{column}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            width=\textwidth, height=5.75cm,
            axis lines=middle, axis line style={-{Latex}},
            xmin=-10, xmax=24, ymin=-0.2, ymax=2.0,
            xtick={0,16}, xticklabels={$0$, $\omega_s$},
            ytick=\empty, xlabel={$\omega$}, ylabel={$X_p(j\omega)$}, clip=false,
            xlabel style={anchor=west, xshift=10pt},
            ylabel style={yshift=10pt, anchor=north},
            declare function={amp(\x)=1.3*exp(-(\x*\x)/6);}
          ]
          \fill[PrimaryColor!10] (axis cs:-6,0) rectangle (axis cs:6,1.45);
          \addplot[ycomb, mark=triangle*, thick, PrimaryColor, samples=7, domain=0:6, mark size=2pt] {amp(x)};
          \addplot[ycomb, mark=triangle*, thick, AccentColor, samples=6, domain=-6:-1, mark size=2pt] {amp(x)};
          \addplot[ycomb, mark=triangle*, thick, PrimaryColor!35, samples=7, domain=16:22, mark size=2pt] {amp(x-16)};
          \addplot[ycomb, mark=triangle*, thick, AccentColor!35, samples=6, domain=10:15, mark size=2pt] {amp(x-16)};
          \draw[<->, thick, gray] (axis cs:6,1.72) -- node[above] {gap: no overlap} (axis cs:10,1.72);
        \end{axis}
      \end{tikzpicture}
    \end{column}
  \end{columns}
...
\begin{frame}{The Same Spectrum from the Sampled Sequence}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \small
      From the \textbf{Recap on DFT} derivation, the sampled periodic signal itself has the Fourier series
      \begin{equation*}
        x_p(t)=\sum_{\ell=-\infty}^{\infty} c_\ell e^{j\ell\omega_0 t},
        \qquad
        c_\ell=\frac{1}{T_0}\sum_{n=0}^{N-1} x[n]e^{-j\frac{2\pi}{N}\ell n}
        =\frac{1}{T_0}X[\ell].
      \end{equation*}
      Since $c_{\ell+N}=c_\ell$, choose any $N$ consecutive indices $\mathcal{K}$ and write $\ell=k+rN$:
      \begin{align*}
        X_p(j\omega)
        &= 2\pi\sum_{\ell=-\infty}^{\infty} c_\ell \delta(\omega-\ell\omega_0) \\
        &= 2\pi\sum_{r=-\infty}^{\infty}\sum_{k\in\mathcal{K}}
        c_{k+rN}\delta(\omega-(k+rN)\omega_0) \\
        &= 2\pi\sum_{r=-\infty}^{\infty}\sum_{k\in\mathcal{K}}
        c_k\delta(\omega-k\omega_0-r\omega_s).
      \end{align*}

      This equation matches exactly with the previous one. Therefore, the coefficients must be related.
    \end{column}
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            ...
            axis lines=middle, axis line style={-{Latex}},
            xmin=-10, xmax=24, ymin=-0.2, ymax=2.05,
            xtick={0,16}, xticklabels={$0$, $\omega_s$},
            ytick=\empty, xlabel={$\omega$}, ylabel={$X_p(j\omega)$}, clip=false,
            xlabel style={anchor=west, xshift=10pt},
            ylabel style={yshift=10pt, anchor=north},
          ...
          \addplot[ycomb, mark=triangle*, thick, PrimaryColor!35, samples=7, domain=16:22, mark size=2pt] {amp(x-16)};
          \addplot[ycomb, mark=triangle*, thick, AccentColor!35, samples=6, domain=10:15, mark size=2pt] {amp(x-16)};
          \node[font=\scriptsize, align=center] at (axis cs:6.7,1.76) {same copy locations,\\now labeled by $c_k$};
        \end{axis}
      \end{tikzpicture}

      \vspace{0.12cm}
      \begin{conceptbox}{Coefficient Match}
        \vspace{-0.2cm}
        \begin{equation*}
          c_k=\frac{a_k}{T}, \qquad X[k]=T_0c_k=\frac{T_0}{T}a_k=Na_k
        \end{equation*}
      \end{conceptbox}
      {\footnotesize Hence, if the periodic signal is bandlimited to the Nyquist band, the DFT bins are scaled versions of the original Fourier series coefficients.}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Reconstruction Gives $x(t)$ and the IDFT}
  \begin{columns}[T]
    \begin{column}{0.39\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            ...
            axis lines=middle, axis line style={-{Latex}},
            xmin=-8, xmax=8, ymin=-0.2, ymax=2.2,
            xtick={-4,0,4}, xticklabels={$-\omega_s/2$, $0$, $\omega_s/2$},
            ytick=\empty, xlabel={$\omega$}, ylabel={$T X_p(j\omega)$}, clip=false,
            xlabel style={anchor=west, xshift=10pt},
            ylabel style={anchor=south},
            declare function={amp(\x)=1.3*exp(-(\x*\x)/6);}
          ]
          \draw[very thick, SecondaryColor] (axis cs:-4,1.2) -- (axis cs:-4,1.5) -- (axis cs:4,1.5) -- (axis cs:4,1.2);
          \addplot[ycomb, mark=triangle*, thick, PrimaryColor, samples=5, domain=0:4, mark size=2pt] {amp(x)};
          \addplot[ycomb, mark=triangle*, thick, AccentColor, samples=4, domain=-4:-1, mark size=2pt] {amp(x)};
          \node[font=\scriptsize] at (axis cs:0,1.8) {integrate one passband};
        \end{axis}
      \end{tikzpicture}
    \end{column}
    ...
    \begin{column}{0.57\textwidth}
      \footnotesize
      Inside one passband, only the non-overlapping baseband harmonics remain:
      \begin{align*}
        x(t)
        &= \frac{1}{2\pi}\int_{-\omega_s/2}^{\omega_s/2} T X_p(j\omega)e^{j\omega t}\,d\omega \\
        &= T\sum_{|k|<N/2} c_k e^{jk\omega_0 t}
        = \sum_{|k|<N/2} a_k e^{jk\omega_0 t}.
      \end{align*}
      Thus the ideal LPF perfectly recovers the continuous-time periodic signal whenever Nyquist holds.

      Sampling at $t=nT$ and using $T_0=NT$ gives
      \begin{align*}
        x[n]=x(nT)
        &= T\sum_{|k|<N/2} c_k e^{jk\omega_0 nT} \\
        &= \frac{1}{N}\sum_{|k|<N/2} X[k]e^{j\frac{2\pi}{N}kn}.
      \end{align*}
      Using the periodicity of $X[k]$, we can shift the index set to $k=0,\dots,N-1$:
      \begin{conceptbox}{Inverse DFT}
        \vspace{-0.2cm}
        \begin{equation*}
