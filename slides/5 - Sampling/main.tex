...
\begin{frame}{DTFT Revisited from the Sampled Signal}
  Earlier in the Nyquist proof we derived the CTFT of the sampled impulse train:
  \begin{equation*}
  ...
  \begin{columns}[T]
        \begin{column}{0.47\textwidth}
            ...
            \begin{itemize}
                ...
                \item This is exactly the physical-frequency sampled spectrum from the Nyquist proof.
    ...
    \begin{column}{0.49\textwidth}
      ...
      \begin{tikzpicture}
            ...
            axis lines=middle, axis line style={-{Latex}},
            xmin=-10.5, xmax=10.5, ymin=-0.25, ymax=2.15,
            xtick={-8,-4,0,4,8},
          ...
          \fill[PrimaryColor!10] (axis cs:-4,0) rectangle (axis cs:4,1.45);
          \addplot[samples=180, domain=-10:10, very thick, PrimaryColor] {xp(x)};
          \draw[<->, thick, gray] (axis cs:-4,1.72) -- node[above] {one period $=\omega_s$} (axis cs:4,1.72);
          \node[font=\scriptsize, PrimaryColor!80!black] at (axis cs:0,1.55) {one physical period};
          \node[font=\scriptsize, AccentColor] at (axis cs:7.2,0.8) {shifted copy};
          \node[font=\scriptsize, AccentColor] at (axis cs:-7.2,0.8) {shifted copy};
        \end{axis}
      \end{tikzpicture}
    \end{column}
    ...
    \begin{column}{0.51\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          \node[SecondaryColor, font=\bfseries] at (axis cs:0,2.08) {ideal LPF keeps this band};
          \node[font=\scriptsize] at (axis cs:0,1.56) {original copy};
          \node[font=\scriptsize, AccentColor] at (axis cs:7.2,0.9) {shifted copy};
          \node[font=\scriptsize, AccentColor] at (axis cs:-7.2,0.9) {shifted copy};
        \end{axis}
      \end{tikzpicture}
    \end{column}
...
\begin{frame}{Inverse DTFT from the Ideal LPF}
  \begin{columns}[T]
    \begin{column}{0.38\textwidth}
      \begin{itemize}
        ...
        \item Ideal reconstruction uses one $\omega_s$-wide period of the sampled spectrum:
          \begin{equation*}
            x(t)=\frac{1}{2\pi}\int_{-\omega_s/2}^{\omega_s/2} T X_p(j\omega)e^{j\omega t}\,d\omega
          \end{equation*}
        \item At the sample locations,
          \begin{equation*}
            x[n]=x(nT).
          \end{equation*}
        \item After the same normalization, we simply read this as the discrete-time sequence.
      \end{itemize}
      ...
      \begin{center}
        \begin{tikzpicture}[>=Latex]
          ...
          \draw[->, thick] (1.9,0) -- node[above] {$\omega T \mapsto \omega$} (5.3,0);
          \node at (6.8,0) {$[-\pi,\pi]$};
        \end{tikzpicture}
      \end{center}
    \end{column}
    ...
    \begin{column}{0.58\textwidth}
      ...
      \begin{align*}
        x[n]=x(nT)
        &= \frac{1}{2\pi}\int_{-\omega_s/2}^{\omega_s/2}
        T X_p(j\omega)e^{j\omega nT}\,d\omega \\
        &\xrightarrow{\ \omega T \mapsto \omega\ }
    ...
    \begin{column}{0.53\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          \draw[<->] (axis cs:0,-0.5) -- node[below] {$\omega_s/N$} (axis cs:0.5,-0.5);
          \draw[<->, thick, PrimaryColor] (axis cs:0,-0.24) -- node[below, font=\scriptsize] {positive} (axis cs:4,-0.24);
          \draw[<->, thick, AccentColor] (axis cs:4,-0.24) -- node[below, font=\scriptsize, align=center] {wrapped\\negative} (axis cs:8,-0.24);
...
\begin{frame}{The Same Spectrum from the Sampled Sequence}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \small
      From the recap derivation, the sampled periodic signal itself has the Fourier series
      \begin{equation*}
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{conceptbox}{Coefficient Match}
        ...
        \end{equation*}
      \end{conceptbox}
      {\footnotesize The comparison is made on the non-overlapping baseband harmonics. If $T=1$, this reads $X[k]=T_0a_k$.}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{column}{0.39\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          \draw[very thick, SecondaryColor] (axis cs:-4,1.7) -- (axis cs:-4,2.0) -- (axis cs:4,2.0) -- (axis cs:4,1.7);
          ...
          \node[font=\scriptsize] at (axis cs:0,2.1) {integrate one passband};
        \end{axis}
      \end{tikzpicture}
    \end{column}
