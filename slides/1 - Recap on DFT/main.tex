...
\end{frame}
\begin{frame}{Why the DFT Works for Periodic Signals}
  \begin{minipage}{0.48\textwidth}
    \begin{conceptbox}{Periodic-Signal View}
      \vspace{-0.2cm}
      \begin{equation*}
      ...
      \vspace{-0.4cm}
      \begin{equation*}
        \omega_k=\frac{k\omega_s}{N}=k\omega_0
      \end{equation*}
    \end{conceptbox}
    \vspace{0.15cm}
    \begin{itemize}
      \item A continuous-time periodic signal has a \textbf{line spectrum}: only harmonic spikes at integer multiples of $\omega_0$.
      \item This is the same periodic-recovery viewpoint revisited later in Sampling: if $N=T_0/T_s$, then $\omega_s=N\omega_0$.
      \item Hence $\omega_k=\frac{k\omega_s}{N}=k\omega_0$, so the DFT bins land \textbf{exactly on the harmonic lines}.
      \item That is why the DFT isolates the $N$ unique Fourier-series coefficients so cleanly in the periodic case.
    \end{itemize}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\textwidth}
    \begin{center}
      \begin{tikzpicture}[scale=0.9]
        \node at (0,3.3) {one period of samples};
        ...
        \node at (5,3.3) {$N$ unique coeffs};
        \node[font=\scriptsize, align=center] at (2.5,2.15) {bins land on\\harmonic lines};
        ...
        \begin{axis}[
            ...
            width=4cm, height=3.8cm,
            axis lines=middle, axis line style={shorten >=-5pt, shorten <=-5pt, -{Latex}},
            xmin=-1, xmax=8.5, ymin=-0.5, ymax=8.5,
            xtick=\empty, ytick=\empty, clip=false
          ]
          \addplot[ycomb, mark=triangle*, PrimaryColor, thick, mark size=2pt] coordinates {
            (0,0.8) (1,2.0) (2,4.0) (3,6.8) (4,7.6) (5,6.8) (6,4.0) (7,2.0)
          };
          \node[font=\scriptsize] at (axis cs:4.2,7.9) {$X[k]\propto a_k$};
        \end{axis}
      \end{tikzpicture}
    \end{center}
  \end{minipage}
...
\begin{frame}{What about Aperiodic Signals?}
  ...
  \begin{itemize}
    ...
    \item Now switch to the practical \textbf{finite-capture} setting used in Sampling's ``Connection to the DFT'' section: an aperiodic signal $x(t)$ observed only over a finite interval $T_0$.
    \item Uniformly sampling this yields a finite-length sequence $x[n]$ that is nonzero only for $0 \le n \le N-1$.
    \item Unlike the periodic case, the associated spectrum is \textbf{continuous} rather than isolated Fourier-series spikes.
    \item Using the pulse train sampling method with sampling period $T$, we can represent the sampled signal as:
      \begin{equation*}
...
\begin{frame}{From the Sampled Signal to the DTFT}
  ...
  \vspace{0.05cm}
  \begin{conceptbox}{CTFT of the sampled signal}
    \vspace{-0.2cm}
    \begin{equation*}
  ...
  \begin{itemize}
      \setlength{\itemsep}{0.2em}
    \item This formula is written in the physical frequency variable $\omega$ and is exactly the repeated sampled-spectrum picture we will revisit in Sampling.
    \item Now compress the frequency axis by the sampling period and reuse the symbol, i.e. apply the mapping $\omega T \mapsto \omega$.
...
\begin{frame}{The Path to the DFT (Aperiodic Signals)}
  \begin{minipage}{0.5\textwidth}
    \begin{itemize}
        \setlength{\itemsep}{0.2em}
      \item In Sampling's ``Connection to the DFT'' section we showed that the DFT simply samples the CTFT of the impulse train:
        \begin{equation*}
          X[k] = X_p\left(j\frac{k\omega_s}{N}\right), \qquad \omega_s=\frac{2\pi}{T}.
        \end{equation*}
      \item Under the same mapping $\omega T \mapsto \omega$, this becomes the normalized DTFT statement:
        \begin{equation*}
    ...
    \begin{center}
      \begin{tikzpicture}[scale=0.9]
        \begin{axis}[
            ...
            xtick={0, 3.1416, 6.2832}, xticklabels={$0$, $\omega_s/2$, $\omega_s$},
            ytick=\empty,
          ...
          \draw[<->] (axis cs:0,-0.5) -- node[below] {$\frac{\omega_s}{N}$} (axis cs:0.5236,-0.5);
