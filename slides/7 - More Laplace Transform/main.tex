...
\begin{frame}[t]{Time Scaling}
  ...
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
      ...
      \begin{itemize}
          \scriptsize
        \item If $0<a<1$, the signal is stretched in time while the ROC is compressed.
        \item If $a>1$, the signal is compressed in time while the ROC is stretched.
        \item If $a<0$, the signal is flipped in time and scaled, same case for the ROC.
      \end{itemize}
    \end{column}
    \begin{column}{0.47\textwidth}
      \vspace{0.5cm}
